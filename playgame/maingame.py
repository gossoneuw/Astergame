...
def score_check_menu():
    """Henter og returnerer highscore fra eksisterende score_check funksjon."""
    with open("high_score.txt", "r") as file:
        old_score = file.read()
        file.close()
        old_score = int(old_score)
...
def main_menu():
    menu = True
    high_score = score_check_menu()  # Henter highscore fra fil

    while menu:
        screen.blit(background, (0, 0))  # Tegner bakgrunnsbildet
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if start_button.collidepoint(event.pos):
                    menu = False  # Lukker menyen og starter spillet
